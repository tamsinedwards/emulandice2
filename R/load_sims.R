...
load_sims <- function(variable, source = NA, region = NA) { # dataset
  ...
  cat(paste("load_sims: reading",variable,"simulation data\n\n"), # dataset
    ...
    if (variable == "ice") {
      ...
      if (source == "GIS") {
        data_file <- paste0( inputs_preprocess, "/GIS/GIS_SIMULATIONS_m_SLE_2014_260819.csv" )
        if (deliverable_test) data_file <- paste0( inputs_preprocess, "/GIS/SLE_SIMULATIONS_GIS_p9_240210.csv")
      }

      if (source == "AIS") {
        data_file <- paste0( inputs_preprocess, "/AIS/AIS_SIMULATIONS_", region, "_m_SLE_2014_260819.csv")
      ...
      if (source == "GLA") {
        ...
        # Regional file
        data_file <- paste0( inputs_preprocess, "/GLA/GLA_SIMULATIONS_",region,"_mm_SLE_2014_260820.csv")
